...
import cc.arduino.contributions.packages.ContributedPlugin;
import cc.arduino.contributions.ui.InstallerTableCell;
import cc.arduino.utils.ReverseComparator;

@SuppressWarnings("serial")
public class ContributedPluginTableCellEditor extends InstallerTableCell {

  private ContributedPluginTableCellJPanel cell;
  private ContributedPluginReleases value;
  ...
  public Component getTableCellEditorComponent(JTable table, Object _value,
                                               ...
                                               int column) {
    value = (ContributedPluginReleases) _value;

    cell = new ContributedPluginTableCellJPanel();
    ...
    cell.downgradeButton.addActionListener(e -> {
      ContributedPlugin selected = (ContributedPlugin) cell.downgradeChooser
          .getSelectedItem();
      onInstall(selected, value.getInstalled());
    });
    cell.versionToInstallChooser.addItemListener(e -> value
        .select((ContributedPlugin) cell.versionToInstallChooser
            .getSelectedItem()));

    setEnabled(true);

    final ContributedPlugin installed = value.getInstalled();

    List<ContributedPlugin> releases = new LinkedList<>(value.releases);
    List<ContributedPlugin> uninstalledReleases = releases.stream()
        .filter(new InstalledPredicate().negate()).collect(Collectors.toList());

    List<ContributedPlugin> installedBuiltIn = releases.stream()
    ...
    final List<ContributedPlugin> uninstalledPreviousReleases = new LinkedList<>();
    final List<ContributedPlugin> uninstalledNewerReleases = new LinkedList<>();
  ...
  protected void onRemove(ContributedPlugin contributedPlugin) {
    // Empty
  }

  protected void onInstall(ContributedPlugin contributedPlugin,
                           ContributedPlugin installed) {
    // Empty
  }
