...
public class ContributionIndexPluginsTableModel
    extends FilteredAbstractTableModel<ContributedPlugin> {

  private final List<ContributedPluginReleases> contributions = new ArrayList<>();
  private final String[] columnNames = { "Description" };
  private final Class<?>[] columnTypes = { ContributedPlugin.class };

  public void updateIndexFilter(String[] filters,
                                Stream<Predicate<ContributedPlugin>> additionalFilters) {
    contributions.clear();
    Predicate<ContributedPlugin> filter = additionalFilters
        .reduce(Predicate::and).get();
    for (ContributedPackage pack : BaseNoGui.indexer.getPackages()) {
      for (ContributedPlugin platform : pack.getPlugins()) {
        if (platform == null) {
          continue;
        }
  ...
  private void addContribution(ContributedPlugin platform) {
    for (ContributedPluginReleases contribution : contributions) {
    ...
    contributions.add(new ContributedPluginReleases(platform));
  }
  ...
  public Object getValueAt(int row, int col) {
    ...
    ContributedPluginReleases contribution = contributions.get(row);
  ...
  public ContributedPluginReleases getReleases(int row) {
    return contributions.get(row);
  }

  public ContributedPlugin getSelectedRelease(int row) {
