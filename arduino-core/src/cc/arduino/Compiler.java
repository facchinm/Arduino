...
import cc.arduino.core.rpc.Common.TaskProgress;
...
public class Compiler implements MessageConsumer {
	...
	private void callArduinoBuilder(TargetBoard board, String vidpid, BuilderAction action, OutputStream outStream, OutputStream errStream) throws RunnerException, IOException {

		CompileReq.Builder req = CompileReq.newBuilder();
		req.setFqbn(board.getFQBN());
		...
		req.setSketchPath(pathToSketch.getParent());

		if (action == BuilderAction.DUMP_PREFS) {
			req.setShowProperties(true);
		}
		...
		.entrySet()
		.stream()
		.forEachOrdered(kv -> req.setBuildProperties(idx[0]++ , kv.getKey() + "=" + kv.getValue()));

		TaskProgress result = null;

		Iterator<CompileResp> stream = BaseNoGui.backend().compile(req);
		while (stream.hasNext()) {
			CompileResp resp = stream.next();
			ByteString out = resp.getOutStream();
			if (out != null)
				outStream.write(out.toByteArray());
			ByteString err = resp.getErrStream();
			if (err != null)
				errStream.write(err.toByteArray());
			result = resp.getTaskProgress();
		}
